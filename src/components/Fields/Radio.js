...
function Radio(props) {
  return (
        <div className="lg:flex lg:items-center text-charcoal-grey-900 py-3">
            <label className="lg:w-36  font-medium mr-1">{props.label}</label>
            {props.options.map(el => (
                <div key={el.id} className="pt-3 lg:pt-0">
                    <div className="flex items-center mr-6">
                        ...
                        <label htmlFor={el.value} className="flex items-center cursor-pointer font-medium text-slate-400 focus:outline-none">
                            <button className="w-14 h-14 bg-slate-100 inline-flex items-center justify-center mr-5 rounded-full flex-no-shrink">
                                <img alt="icon" className="w-7 h-7" src={(props.value === el.value) ? el.active : el.icon} />
                            </button>
                            {el.label}
                        </label>
                    </div>
